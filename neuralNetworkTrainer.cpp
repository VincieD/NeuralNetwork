...
void neuralNetworkTrainer::trainNetwork( trainingDataSet* tSet )
{
	...
	{			
		...
		//cout << std::fixed << std::setprecision(3) << NN->wInputHidden[0] << " " << NN->wInputHidden[1] << " " << NN->wInputHidden[2] << " " << NN->wInputHidden[3] << " " << NN->wInputHidden[4] << " " << NN->wInputHidden[5] << endl;
...
void neuralNetworkTrainer::runTrainingEpoch( vector<dataEntry*> trainingSet )
{
	...
	{						
		...
		//slWinA.matrixPrintOut(trainingSet[tp]->pattern, 8);
		//system("PAUSE");
		//cout << "Im in runTrainingEpoch... ";
		backpropagate( trainingSet[tp]->target );
		//cout << trainingSet[tp]->target[0] << endl;
		cout << "Trying to backpropagate picture... " << tp << " from " << trainingSet.size() << " is done." << endl;
...
********************************************************************/
void neuralNetworkTrainer::backpropagate(vector<double>& desiredOutputs)
{		
	...
	for (int k = 0; k < NN->nOutput; k++)
	{
		...
		//cout << getOutputErrorGradient(desiredOutputs[k], NN->outputNeurons[k]) << " ";
	...
	cout << std::fixed << std::setprecision(3) << NN->outputNeurons[0] << " " << NN->outputNeurons[1] << " " << NN->outputNeurons[2] << " ";
...
void neuralNetworkTrainer::updateWeights()
{
	...
	//cout << "Here i should set a break point";
